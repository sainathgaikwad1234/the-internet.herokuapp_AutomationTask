...
test('Patient Registration – Mandatory Fields', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'First Name *' }).fill('Sainath');
  await page.getByRole('textbox', { name: 'Last Name *' }).fill('Gaikwad');
  await page.getByRole('textbox', { name: 'Date Of Birth *' }).fill('04-06-2002');
  ...
  await page.getByRole('textbox', { name: 'Email *' }).fill('sainath.gaikwad+1@thinktive.com');
  ...
  // await page.getByPlaceholder('Find Patient').fill('Sainath Gaikwad');
  await expect(page.locator('text=Sainath Gaikwad')).toBeVisible({ timeout: 10000 });
});
